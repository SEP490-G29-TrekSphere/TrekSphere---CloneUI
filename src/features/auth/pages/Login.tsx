...
export default function Login() {
  ...
  return (
    <>
      ...
      {(isSubmitting || isGoogleLoading || isRedirecting) && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background/60 backdrop-blur-sm transition-all duration-300">
          <AppSpinner size="xl" className="text-primary mb-4" />
          <p className="text-lg font-semibold text-primary animate-pulse">
            {isRedirecting ? 'Đang vào hệ thống...' : 'Đang xử lý đăng nhập...'}
          </p>
        </div>
      )}
